...
export default function StudentScreen() {
    ...
    const ImageButton = ({ src, alt, name, englishName, onClick }: ImageButtonProps) => (
        <Flex style={{ position: 'relative', width: '14vw', border: '2px solid white' }}>
            <img
                src={src}
                alt={alt}
                style={{
        ...
        <Flex id="student" className="parent-200vh" flexDir={'column'}>
                ...
                {contentVisible &&
                    ...
                    >
                        <Flex justifyContent={'center'} alignItems={'center'} ml={'13vw'} mr={'1vw'}>
                            ...
                            <HStack ref={studentRef} spacing={2} overflowX="scroll" h={'80vh'} p={'2%'} maxW={'71vw'} sx={{
                                '::-webkit-scrollbar': {
                                    display: 'none',
                                },
                            }}>
                                <Flex flexDir={"column"} >
                                    <Flex >
                                        <div style={{ width: '14vw', height: '30vh', borderTop: '2px solid white', flexShrink: 0 }}>
                                        ...
                                        {videoStudentData.slice(0, Math.ceil(videoStudentData.length / 2 - 1)).map((student, index) => (
                                            <ImageButton src={student.profile} alt="SignLogo" name={student.name} englishName={student.englishName} onClick={() => handleStudent(student)} />
                                        )
                                        )}
                                    </Flex>
                                    <Flex>
            ...
            <div >
                {contentVisible &&
                    ...
                    >
                        <Flex justifyContent={'center'} alignItems={'center'} ml={'13vw'} mr={'1vw'}>
                            <button onMouseEnter={() => handleArrowMouseEnter2('left')} onMouseLeave={handleArrowMouseLeave2}
                                style={{ fontSize: '5vw', color: isScrolledToLeft2 ? 'transparent' : 'white', height: '60vh', zIndex: 10 }} >
                                <ChevronLeftIcon />
                            </button>
                            <HStack ref={studentRef2} spacing={2} overflowX="scroll" h={'80vh'} p={'2%'} maxW={'71vw'} sx={{
                                '::-webkit-scrollbar': {
                                    display: 'none',
                                },
                            }}>
                            ...
                            <button onMouseEnter={() => handleArrowMouseEnter2('right')} onMouseLeave={handleArrowMouseLeave2}
